 ...
 * page: the fee bound, the dynamic-fee-needs-a-hook rule, the return-delta
 * rule, the token labels, and that token0 sorts before token1.
 *
 * Returning `null` rather than throwing is what lets a caller drop one entry
 * and publish the rest.
 */
export const normalizeV4PoolCard = (raw: RawV4PoolCard): V4PoolCard | null => {
  const pool = normalizeV4PoolEntity(raw);
