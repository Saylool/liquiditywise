...
Do not write numbers. Not prices, not ticks, not percentages, not day counts, not dates. The interface displays every figure next to your text, so refer to them instead: "the range shown above", "the volatility figure", "the window this was measured over". A number you write is a number that can disagree with the one beside it, and the reader has no way to tell which is right. That is as true of a number spelled out in words as of one written in digits, in every language: "eighty-five of the ninety days" and "oitenta e cinco dos noventa dias" state two figures exactly as "85 of 90 days" does. Say what the figures show instead — "most of the days checked", "nearly all of the window". The only digits permitted are the protocol's own version names, v3 and v4.
